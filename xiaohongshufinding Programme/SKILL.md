...
## AI 编排流程（外部打分 + 外部生图 + 草稿）

1. 用户配置 `config/external_ai.json`（从 `config/external_ai.json.example` 复制），打分/生图 URL 与请求头由用户填写；可用环境变量占位 `${VAR}`。
2. 执行 `python scripts/ai_content_pipeline.py fetch-search --keyword "..." --output /abs/search.json` 导出检索结果（需主页已登录）。
3. 在外部 Agent 中读取 `search.json`，调用用户自选的 LLM 产出打分 JSON，或使用 `score-api`（若已启用 `score_api`）由服务端合并；亦可用 `merge-scores --scores-json` 将 `feed_scores` 写入每条 `feed` 的 `ai_score` / `ai_labels` / `ai_reason`。
4. 由外部 LLM 生成最终 `title.txt`、`content.txt`；生图提示写入 `prompt.txt`。
5. 执行 `image-api --count 1..4 --prompt-file ... --output /abs/image_urls.json`（若启用 `image_api`），或用户直接提供 1–4 个高清图 URL。
6. 执行 `save-draft`（默认等价 `--preview`）或 `publish_pipeline.py --preview ...`，将内容写入创作者中心草稿；**禁止在用户未确认前执行非预览发布**。
