...
- [启动应用](#启动应用)
  ...
  - [方式三：一键启动脚本（Windows）](#方式三一键启动脚本windows)
- [使用教程](#使用教程)
  - [1. 启动浏览器](#1-启动浏览器)
  - [2. 登录小红书](#2-登录小红书)
  - [3. 搜索笔记](#3-搜索笔记)
...
| 历史记录持久化 | 搜索快照、报告列表、AI 工作台状态写入 `redbook_session.json`；可在「设置」指定目录（默认 `tmp/redbook_history/`） |
...
在项目**根目录**双击 **`Redbook.bat`**，或在 `scripts` 下使用 **`start_redbook_desktop.bat`**（相对路径，可随仓库移动）。

脚本会依次：启动 `serve_local_app.py`、启动 `desktop` 的 `npm run dev`、尝试打开 `http://localhost:5173`（若 Vite 占用其他端口，请以终端提示为准）。

未配置虚拟环境时，请确保 `python`、`npm` 已在 PATH 中；若使用 `.venv`，根目录存在 `.venv\Scripts\activate.bat` 时会自动激活。
...
**历史记录保存位置：**
- 可指定本机文件夹（如 `E:\VS code\history`），用于保存 `redbook_session.json`（当前搜索快照、报告历史、AI 工作台状态等）
- 留空则使用项目内默认目录 `tmp/redbook_history/`
- 目录指针保存在 `tmp/desktop_ui_settings.json`（勿提交敏感路径若介意）
...
├── Redbook.bat                 # Windows 一键启动（根目录）
├── scripts/start_redbook_desktop.bat  # 同上，位于 scripts 下
├── tmp/                        # 本地缓存（gitignore）；含 desktop_ui_settings、redbook_history 等
├── SKILL.md                    # AI Agent Skill 定义
├── AGENTS.md                   # 仓库开发指南
├── docs/technical-interview-guide.md  # 技术架构与面试说明
└── README.md                   # 本文件
```
...
若需准备**技术面试**或向他人讲解实现原理，请参阅 **[docs/technical-interview-guide.md](docs/technical-interview-guide.md)**（系统架构、CDP、FastAPI、Electron、AI 层、安全与常见追问）。
...
- `config/ai_settings.json` 已在 **`.gitignore`** 中，正常 `git push` 不会带上该文件；新克隆仓库请复制 `config/ai_settings.json.example` 再填写 Key
- 若你**曾经**把带真实 Key 的 `ai_settings.json` 推送到远程，Git 历史里仍可能留存，请在服务商控制台**作废并更换 API Key**
- 不要将 `config/accounts.json` 等包含敏感信息的文件提交到 Git（亦已在 `.gitignore`）
