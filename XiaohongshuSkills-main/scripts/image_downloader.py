...
class ImageDownloader:
    ...
    def download_all(self, urls: list[str], *, strict: bool = False) -> list[str]:
        """
        Download multiple images. Returns list of local file paths.

        By default, skips URLs that fail to download (logs the error, continues).
        When strict=True, raises RuntimeError on the first failed download.
        """
        paths: list[str] = []
        for url in urls:
            try:
                ...
                if strict:
                    raise RuntimeError(f"Strict download failed for {url!r}: {e}") from e
        return paths
