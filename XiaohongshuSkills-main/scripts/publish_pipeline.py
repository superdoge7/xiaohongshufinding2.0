...
def main():
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "When using --image-urls, fail immediately if any URL cannot be downloaded "
            "(default: skip failed URLs and continue if at least one succeeds)."
        ),
    )
    ...
    if is_video_mode:
        ...
        downloader = ImageDownloader(temp_dir=args.temp_dir)
        try:
            image_paths = downloader.download_all(
                args.image_urls, strict=args.strict_image_downloads
            )
